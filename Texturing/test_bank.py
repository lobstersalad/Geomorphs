...
# Find dungeon walls with canny edge detection
tight = cv2.Canny(dilated, 225, 250)
cv2.imwrite('cannytight_test_dungeon.png', tight)
wide = cv2.Canny(dilated, 10, 200)
cv2.imwrite('cannywide_test_dungeon.png', wide)

# Find dungeon walls by drawing contours
(thresh, dilated_bw) = cv2.threshold(dilated_bw, 128, 255, 0)
cv2.imwrite('bw_test_dungeon.png', dilated_bw)
...
cv2.imwrite('cnt_test_dungeon.png', dilated)
...
cv2.imwrite('test_dungeon_doors.png', img)

# Use template matching to find floor tiles
img_bw = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
template = cv2.imread('floor_tile_template.png', 0)
w, h = template.shape[::-1]
res = cv2.matchTemplate(img_bw, template, cv2.TM_CCOEFF_NORMED)
threshold = 0.8
loc = np.where( res >= threshold)
for pt in zip(*loc[::-1]):
    cv2.rectangle(img, pt, (pt[0] + w, pt[1] + h), (0,0,255), 2)
cv2.imwrite('test_dungeon_floor_tiles.png', img)
